...
window.onload = function() {
    ...
    function meteo(element, ville) {
        ...
        window.fetch(chaine)
            ...
            .then(resJson => {
                a = resJson;
                temp = a['main']['temp'];
                element.textContent = String(Math.round(temp)) + "°C"; // Affiche la temperature 
                console.log(ville + String(temp) + "°C"); // Verifie que le code marche
            })
    }
    ...
    */
    ...
    for (var i = 0; i < destinations.length; i++) {
        ...
        document.getElementById("all").appendChild(content) //Ajout du template avec les informations
